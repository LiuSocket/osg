...
//#include "IOSVideoImageStream.hpp"
...
"    void main()                                                \n"
"    {                                                          \n"
"      gl_FragColor = texture2D(tex0, tex_coord0);              \n"
...
- (void)applicationDidFinishLaunching:(UIApplication *)application {
    ...
    UIView *subView =  [[UIView alloc] initWithFrame:CGRectInset(self._window.bounds,0,0)];
...
//    path = [NSString stringWithFormat:@"@s.iosvideo", path];
    NSLog(@"path = %@", path);
    osg::Geode* geode = new osg::Geode();
    osg::ShapeDrawable* drawable = new osg::ShapeDrawable(new osg::Box(osg::Vec3(1,1,2), 1));
    ...
    //osgIOSVideo::IOSVideoImageStream* image = new osgIOSVideo::IOSVideoImageStream();
//    image->open([path UTF8String]);
    //    texture->setImage(image);
    texture->setImage(osgDB::readImageFile([path UTF8String]));
    ...
    osg::Shader * vshader = new osg::Shader(osg::Shader::VERTEX, DEFAULT_VERTEX_SHADER);
    osg::Shader * fshader = new osg::Shader(osg::Shader::FRAGMENT, DEFAULT_FRAGMENT_SHADER);
