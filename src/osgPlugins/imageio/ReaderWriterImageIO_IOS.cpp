...
public:
    ...
    {
        // Parse out '?' for url requests (such as asset-library)
        std::string fileprotocol("file://");
        std::string full_file_name = file_name;
        if (full_file_name.substr(0, fileprotocol.length()) == fileprotocol) {
            int end = full_file_name.find("?");
            if (end == std::string::npos) {
                end = full_file_name.length();
            }
            full_file_name = full_file_name.substr(fileprotocol.length() - 1, end - fileprotocol.length() + 1);
        } else {
            int end = full_file_name.find("?");
            if (end == std::string::npos) {
                end = full_file_name.length();
            }
            full_file_name = full_file_name.substr(0, end);
        }

        std::string ext = osgDB::getLowerCaseFileExtension(full_file_name.substr(0,full_file_name.find_first_of("?")));
        if (!acceptsExtension(ext)) return ReadResult::FILE_NOT_HANDLED;

        full_file_name = osgDB::findDataFile(full_file_name, the_options );
