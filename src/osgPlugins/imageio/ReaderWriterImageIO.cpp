...
osg::Image* CreateOSGImageFromCGImage(CGImageRef image_ref, bool premultiply)
{
...
public:
   ...
   virtual bool acceptsExtension(const std::string& extension) const
   {
    ...
    int getPremultiply(const osgDB::ReaderWriter::Options *options) const
    {
        if(options) {
    ...
    ReadResult readImageStream(std::istream& fin, const osgDB::ReaderWriter::Options* the_options) const
    {
        ...
        bool premultiply = getPremultiply(the_options);
        // Create an osg::Image from the CGImageRef.
        osg::Image* osg_image = CreateOSGImageFromCGImage(cg_image_ref, premultiply);
    ...
    {
        ReadResult read_result = readImageStream(fin, the_options);
        return read_result;
    }

  ReadResult readImageFile(const std::string& file_name, const osgDB::ReaderWriter::Options* the_options) const
    {
...
#if 1
        ReadResult read_result = readImageFile(full_file_name, the_options);
#else
