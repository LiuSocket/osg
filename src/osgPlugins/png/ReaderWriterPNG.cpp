...
class ReaderWriterPNG : public osgDB::ReaderWriter
{
    public:
        ...
        ReadResult readPNGStream(std::istream& fin, const Options* options) const
        {
            int trans = PNG_ALPHA;
            pngInfo pInfo;
            pngInfo *pinfo = &pInfo;
            ...
            #endif
            {
                ...
                if(getPremultiply(options)) {
                    // Premultiplying data
                    if (pixelFormat == GL_RGBA) {
                        for(int i = 0; i < width * height; i += 4) {
                            float alpha = data[i+3] / 255.0;
                            data[i] *= alpha;
                            data[i + 1] *= alpha;
                            data[i + 2] *= alpha;
                        }
        ...
        {
            return readPNGStream(fin, options);
        }
        ...
        virtual ReadResult readImage(const std::string& file, const osgDB::ReaderWriter::Options* options) const
        {
            ...
            ReadResult rr = readPNGStream(istream, options);
