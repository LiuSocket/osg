...
{
    std::string fontName;
    is.readWrappedString( fontName );

    osgText::Font* font = osgText::readFontFile(fontName);
    //    OSG_NOTICE<<std::endl<<"readFont:"<< fontName << " font=" << font << std::endl<<std::endl;
    text.setFont(font);
    return true;
}
...
{
    std::string fontName = osgDB::getSimpleFileName(text.getFont()->getFileName());
    os.writeWrappedString(fontName);

    //    OSG_NOTICE<<std::endl<<"writeFont"<< text.getFont()->getFileName() <<std::endl<<std::endl;
    os << std::endl;
    return true;
}
